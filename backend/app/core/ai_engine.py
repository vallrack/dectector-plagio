...
class AIEngine:
    ...
    def _is_filler_text(text: str) -> bool:
        """Detects if the text is likely generated by =rand(), =lorem(), or is highly repetitive."""
        lower_text = text.lower()
        
        # Known filler phrases from Word's =rand() and =lorem()
        known_fillers = [
        ...
        if any(filler in lower_text for filler in known_fillers):
            return True
            
        # Repetitiveness check: if a document has many words but very few unique ones
        words = re.findall(r'\b\w{4,}\b', lower_text)
        if len(words) > 50:
            unique_words = len(set(words))
            ratio = unique_words / len(words)
            if ratio < 0.15:  # Highly repetitive (e.g. same paragraph copied 10+ times)
                return True
    ...
    def detect_ai_signature(code: str) -> dict:
        ...
        """
        is_filler = AIEngine._is_filler_text(code)
        ...
        # Override score if the text is just filler / heavily repetitive
        if is_filler:
            base_score = 15.0
